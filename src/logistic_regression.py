...
class LogisticRegression:
    ...
    def fit(
        ...
        X = torch.cat([features, torch.ones((num_samples, 1))], dim=1)

        for epoch in range(epochs):
            z = X @ self._weights
            predictions = self.sigmoid(z) # calculo de predicciones usando sigmoide

            loss = self.binary_cross_entropy_loss(predictions, labels)

            gradient = torch.matmul(X.T, (predictions - labels)) / num_samples

            self._weights -= learning_rate * gradient # actualizar los pesos con el descenso de gradiente
    ...
    def binary_cross_entropy_loss(
        ...
        """
        epsilon = 1e-10
        ce_loss: torch.Tensor = -torch.mean(
            targets * torch.log(predictions + epsilon) +
            (1 - targets) * torch.log(1 - predictions + epsilon)
        )
        return ce_loss
